...
namespace nadjieb {
    namespace utils {
        enum class State {
            UNSPECIFIED = 0, NEW, BOOTING, RUNNING, TERMINATING, TERMINATED
        };
...
namespace nadjieb {
    ...
    public:
        ...
        void start(int port, int num_workers = std::thread::hardware_concurrency()) {
            ...
            listener_.withOnMessageCallback(on_message_cb_).withOnBeforeCloseCallback(on_before_close_cb_).runAsync(
                    port);
    ...
    private:
        ...
        nadjieb::net::OnMessageCallback on_message_cb_ = [&](const nadjieb::net::SocketFD &sockfd,
            ...
            init_res.setValue("Cache-Control",
